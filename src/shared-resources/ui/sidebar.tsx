/* eslint-disable @typescript-eslint/naming-convention, no-param-reassign, @typescript-eslint/no-redeclare */
...
>(({ className, onClick, label, ...props }, ref) => {
  const { toggleSidebar, state } = useSidebar(); // Access `state` from context
  const collapsed = state === 'collapsed'; // Determine if the sidebar is collapsed
  ...
  return (
    <Button
      ...
      size='icon'
      className={cn(
        'flex items-center justify-start gap-x-2 h-12 w-auto px-2 font-medium text-base font',
        className
      )}
      onClick={(event) => {
        onClick?.(event);
